...
const USERS_KEY = 'web-terminal-os-users';
class StorageService {
    ...
    // ── User credentials (stored OUTSIDE the VFS, user-invisible) ──
    loadUsers() {
        const data = localStorage.getItem(USERS_KEY);
        if (data) {
            try { return JSON.parse(data); } catch (e) { return null; }
        }
        // Legacy migration: user.json used to live at VFS root
        const legacy = this.readFile('/user.json');
        if (legacy) {
            try {
                const users = JSON.parse(legacy);
                this.saveUsers(users);
                this.deleteFile('/user.json');
                return users;
            } catch (e) {}
        }
        return null;
